...
def find_guge_sous_on_screen(threshold=0.55):
    """
    在屏幕上查找模板图片位置
    :param threshold: 匹配阈值（0~1）
    :return: 匹配中心坐标 (x, y)，找不到返回 None
    ...
    # 2️⃣ 读取模板
    lis = ['../tutample/white_sous.png',"../tutample/blank_sou.png"]
    ans = ()
    for i in lis:
...
def find_theme_on_screen(threshold=0.7):
    """
    在屏幕上查找模板图片位置
    :param threshold: 匹配阈值（0~1）
    :return: 匹配中心坐标 (x, y)，找不到返回 None
    ...
    # 2️⃣ 读取模板
    template = cv2.imread('../tutample/theme2.png')
    template_gray = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
    w, h = template_gray.shape[::-1]
    ...
    if max_val >= threshold:
        ...
        return None
def find_localtion_on_screen(threshold=0.7):
    """
    在屏幕上查找模板图片位置
    :param threshold: 匹配阈值（0~1）
    :return: 匹配中心坐标 (x, y)，找不到返回 None
    ...
    # 2️⃣ 读取模板
    template = cv2.imread('../tutample/sousdingwei.png')
    template_gray = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
    w, h = template_gray.shape[::-1]
